...
- Добавлен файл settings.ini, через который можно настраивать параметры трейдинга и стратегий для отдельных инструментов
- Часть функционала торгового робота сделана асинхронной
- Добавлен unit-тест для CRUD
- Для функции crud.create_candle() изменено поведение: теперь вместо исполнения запроса через ORM, запрос выполняется вручную, через Core.
Это ускоряет процесс вставки и позволяет избежать некоторых багов
- Добавлен модуль config для парсинга settings.ini
- Добавлен модуль utils_func, в котором содержутся все наиболее часто используемые функции
- Логика торгового робота изменена так, чтобы он мог торговать несколькими торговыми инструментами, а не одним

### 22.05.2024

- Добавлены еще пара unit-тестов для простых утилитных функций

### 22.05.2024 - 23.05.2024

- Реализована механизм стоп-маркет заявок для песочницы
- добавлен метод is_trade_available в InvestBot для определени доступности торговли для инструмента
- добавлен метод get_free_money() для получения суммы свободных денег на счете во float

### 13.06.2024

- Ахренеть как много изменений, все и не описать. Немного улучшил стрим. Добавил графический интерфейс
Короче много всего, что делает работу робота стабильнее. Но еще много работы